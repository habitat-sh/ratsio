...
impl NatsConnection {
    ...
    pub(crate) fn trigger_reconnect(conn: Arc<Self>) {
        ...
        let connect_version = conn.state.read().1;
        {
            ...
            if state_guard.0 == NatsConnectionState::Reconnecting {
                ...
                return;
            } else if state_guard.0 == NatsConnectionState::Connected
                && state_guard.1 > connect_version
            {
    ...
    pub fn create_connection(
        ...
        cluster_uris: &[String],
        tls_required: bool,
    ) -> impl Future<Item = NatsConnection, Error = RatsioError> {
        let cluster_addrs = NatsConnection::parse_uris(cluster_uris);
        let init_hosts = cluster_uris.to_vec();
        NatsConnection::get_conn_inner(cluster_addrs, tls_required).map(move |inner| {
            NatsConnection {
                is_tls: tls_required,
    ...
    pub fn parse_uris(cluster_uris: &[String]) -> Vec<(Url, SocketAddr)> {
        cluster_uris
            ...
            .map(|cluster_uri| {
                ...
                match node_url {
                    Ok(node_url) => match node_url.host_str() {
                        Some(host) => {
                            let host_and_port =
                                format!("{}:{}", &host, node_url.port().unwrap_or(4222));
                            match SocketAddr::from_str(&host_and_port) {
                                Ok(sock_addr) => {
                                    info!(" Resolved {} to {}", &host, &sock_addr);
                                    vec![(node_url.clone(), sock_addr)]
                                }
                                Err(_) => match host_and_port.to_socket_addrs() {
                                    Ok(ips_iter) => ips_iter
                                        .map(|x| {
                                    ...
                                    Err(err) => {
                                        error!(
                                            "Unable resolve url => {} to ip address => {}",
                                            cluster_uri, err
                                        );
                                        Vec::new()
                                    }
    ...
    fn get_conn_inner(
        cluster_addrs: Vec<(Url, SocketAddr)>,
        tls_required: bool,
        ...
        fn get_conn_step(
            cluster_addrs: &[(Url, SocketAddr)],
            tls_required: bool,
        ) -> impl Future<Item = (Url, NatsConnectionInner), Error = RatsioError> {
            if cluster_addrs.is_empty() {
                Either::A(future::err::<(Url, NatsConnectionInner), RatsioError>(
                    RatsioError::NoRouteToHostError,
                ))
            } else {
                Either::B(
                    future::ok(cluster_addrs[0].clone())
                        .and_then(move |(node_url, node_addr)| {
                            if tls_required {
                                match node_url.host_str() {
                                    Some(host) => future::ok(Either::B(
                                        NatsConnection::connect_tls(host.to_string(), node_addr)
                                ...
                                future::ok(Either::A(
                                    NatsConnection::connect(node_addr)
                                        .map(move |con| (node_url.clone(), con)),
                                ))
                            }
                        })
        ...
        Either::B(loop_fn(cluster_addrs, move |cluster_addrs| {
            let rem_addrs = Vec::from(&cluster_addrs[1..]).clone();
            get_conn_step(&cluster_addrs[..], tls_required)
                .and_then(move |inner| Ok(Loop::Break(inner)))
                .or_else(move |_err| {
