...
fn parse_obj1() {
  ...
  \"b\": \"x\"
    ...
    let x: HashMap<_, _> = vec![
        (String::from("b"), JsonValue::String(String::from("x"))),
        (String::from("v"), JsonValue::Number(42.0)),
    ]
    .into_iter()
    .collect();
    assert_eq!(
        value(&test[..]).unwrap(),
        (b"\0" as &[u8], JsonValue::Object(x))
    );
}
...
fn parse_err_operation() {
    ...
    match operation(&input.as_bytes()) {
        Ok(obj) => assert_eq!(
            ...
            (
                b"" as &[u8],
                Op::ERR(String::from("Unknown Protocol Operation"))
            )
        ),
        Err(err) => {
